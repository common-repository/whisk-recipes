...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Libraries\Sidebar_Manager;
...
class Sidebar_Manager
{
    ...
    public function boot()
    {
        ...
        if (\function_exists('Whisk\\Recipes\\Vendor\\crb_get_default_sidebar_options')) {
    ...
    public function execute_action($action, $input)
    {
        ...
        if (empty($name)) {
            return new \Whisk\Recipes\Vendor\WP_Error('name-missing', __('Please enter a name for the sidebar.', 'carbon-fields'));
        }
        $result = new \Whisk\Recipes\Vendor\WP_Error('unknown-action', __('Unknown action attempted.', 'carbon-fields'));
        switch ($action) {
    ...
    public function add_sidebar($name, $id = '')
    {
        ...
        if (isset($registered_sidebars[$id])) {
            return new \Whisk\Recipes\Vendor\WP_Error('sidebar-exists', __('Sidebar with the same ID is already registered.', 'carbon-fields'));
        }
        $registered_sidebars[$id] = array('id' => $id, 'name' => $name);
        $success = update_option('carbon_custom_sidebars', $registered_sidebars);
        if (!$success) {
            return new \Whisk\Recipes\Vendor\WP_Error('update-failed', __('Failed to update option storing your custom sidebars. Please contact support.', 'carbon-fields'));
        }
    ...
    public function remove_sidebar($id)
    {
        ...
        if (isset($registered_sidebars[$id])) {
            ...
            return new \Whisk\Recipes\Vendor\WP_Error('sidebar-not-found', __('Sidebar not found.', 'carbon-fields'));
        }
        $success = update_option('carbon_custom_sidebars', $registered_sidebars);
        if (!$success) {
            return new \Whisk\Recipes\Vendor\WP_Error('update-failed', __('Failed to update option storing your custom sidebars. Please contact support.', 'carbon-fields'));
    ...
    {
        wp_enqueue_style('carbon-sidebar-manager', \Whisk\Recipes\Vendor\Carbon_Fields\URL . '/core/Libraries/Sidebar_Manager/assets/css/app.css', array(), \Whisk\Recipes\Vendor\Carbon_Fields\VERSION);
        wp_enqueue_script('carbon-sidebar-manager', \Whisk\Recipes\Vendor\Carbon_Fields\URL . '/core/Libraries/Sidebar_Manager/assets/js/app.js', array(), \Whisk\Recipes\Vendor\Carbon_Fields\VERSION);
        wp_localize_script('carbon-sidebar-manager', 'crbSidebarl10n', array('add_sidebar' => __('Add Sidebar', 'carbon-fields'), 'enter_name_of_new_sidebar' => __('Please enter the name of the new sidebar:', 'carbon-fields'), 'remove_sidebar_confirmation' => __('Are you sure you wish to remove this sidebar?', 'carbon-fields')));
    }
}
