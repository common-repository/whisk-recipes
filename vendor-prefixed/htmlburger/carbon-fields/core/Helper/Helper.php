...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Helper;
...
class Helper
{
    ...
    {
        Carbon_Fields::verify_fields_registered();
    ...
    public static function set_value($object_id, $container_type, $container_id, $field_name, $value)
    {
        static::with_field_clone($object_id, $container_type, $container_id, $field_name, function ($field) use($container_type, $container_id, $field_name, $value) {
            if (!$field) {
                ...
                \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Could not find a field which satisfies the supplied pattern ' . $container_message . ': ' . $field_name);
                return;
            }
            /** @var \Carbon_Fields\Field\Field $field */
    ...
    public static function get_relation_type_from_array($array, $allowed_relations = array('AND', 'OR'), $relation_key = 'relation')
    {
        ...
        if (!\in_array($relation, $allowed_relations)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid relation type ' . $relation . '. ' . 'The rule should be one of the following: "' . \implode('", "', $allowed_relations) . '"');
        }
        return $relation;
    }
    /**
    ...
    public static function get_attachment_id($url)
    {
        ...
        if (\strpos($url, $dir['baseurl'] . '/') !== \false) {
            ...
            $query = new \Whisk\Recipes\Vendor\WP_Query($query_args);
            if ($query->have_posts()) {
    ...
    public static function input()
    {
        ...
        if (\Whisk\Recipes\Vendor\Carbon_Fields\COMPACT_INPUT) {
    ...
    {
        if (isset($input[\Whisk\Recipes\Vendor\Carbon_Fields\COMPACT_INPUT_KEY])) {
            $inputs = $input[\Whisk\Recipes\Vendor\Carbon_Fields\COMPACT_INPUT_KEY];
