...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Loader;
...
class Loader
{
    ...
    public function __construct(\Whisk\Recipes\Vendor\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager $sidebar_manager, \Whisk\Recipes\Vendor\Carbon_Fields\Container\Repository $container_repository)
    {
    ...
    public function boot()
    {
        ...
        include_once \Whisk\Recipes\Vendor\Carbon_Fields\DIR . '/core/functions.php';
        ...
        \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::service('legacy_storage')->enable();
        # Enable the meta query service
        \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::service('meta_query')->enable();
        # Enable the REST API service
        \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::service('rest_api')->enable();
        # Enable post meta revisions service
        \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::service('revisions')->enable();
    ...
    {
        $dir = \Whisk\Recipes\Vendor\Carbon_Fields\DIR . '/languages/';
    ...
    public function trigger_fields_register()
    {
        ...
        } catch (\Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception $e) {
            $callback = '';
    ...
    protected function enqueue_style($src, $deps = array())
    {
        ...
        wp_enqueue_style('carbon-fields-' . $src, \Whisk\Recipes\Vendor\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.css', $deps, \Whisk\Recipes\Vendor\Carbon_Fields\VERSION);
    }
    /**
    ...
    protected function enqueue_script($src, $deps = array())
    {
        $suffix = $this->get_assets_suffix();
        $context = $this->get_assets_context();
        wp_enqueue_script('carbon-fields-' . $src, \Whisk\Recipes\Vendor\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.js', $deps, \Whisk\Recipes\Vendor\Carbon_Fields\VERSION);
    }
    /**
     * Enqueues the assets.
     *
    ...
    public function enqueue_assets()
    {
        ...
        $revisions = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::service('revisions');
        wp_localize_script('carbon-fields-vendor', 'cf', apply_filters('carbon_fields_config', array('config' => array('locale' => $this->get_ui_translations(), 'pagenow' => $pagenow, 'compactInput' => \Whisk\Recipes\Vendor\Carbon_Fields\COMPACT_INPUT, 'compactInputKey' => \Whisk\Recipes\Vendor\Carbon_Fields\COMPACT_INPUT_KEY, 'revisionsInputKey' => $revisions::CHANGE_KEY))));
    }
    /**
    ...
    public function get_json_data()
    {
        ...
        foreach ($containers as $container) {
            ...
            if (\is_a($container, 'Whisk\\Recipes\\Vendor\\Whisk\\Recipes\\Vendor\\Carbon_Fields\\Container\\Block_Container', \true)) {
                $carbon_data['blocks'][] = $container_data;
            } else {
                $carbon_data['containers'][] = $container_data;
            }
        }
        $carbon_data['sidebars'] = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::get_active_sidebars();
        return $carbon_data;
    }
    /**
    ...
    public function fetch_association_options()
    {
        ...
        $field = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::get_field(null, $container_id, $field_name);
