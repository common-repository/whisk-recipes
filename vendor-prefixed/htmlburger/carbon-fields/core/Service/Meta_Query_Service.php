 ...
 */
class Meta_Query_Service extends \Whisk\Recipes\Vendor\Carbon_Fields\Service\Service
{
    /**
    ...
    protected function filter_meta_query_array($condition, $container_type)
    {
        ...
        $property = isset($condition['carbon_field_property']) ? $condition['carbon_field_property'] : \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY;
        // bail if we cannot find the field
        if ($field !== null) {
            $storage_key = $this->key_toolset->get_storage_key_with_index_wildcards($field->is_simple_root_field(), \array_merge($field->get_hierarchy(), array($field->get_base_name())), $property);
