...
class Revisions_Service extends \Whisk\Recipes\Vendor\Carbon_Fields\Service\Service
{
    ...
    public function add_fields_to_revision($fields, $post = null)
    {
        ...
        if (!\function_exists('Whisk\\Recipes\\Vendor\\get_current_screen')) {
            return $fields;
        }
    ...
    public function update_revision_field_value($value, $field_name, $post = null, $direction = \false)
    {
        ...
        $value = whisk_carbon_get_post_meta($post->ID, \substr($field_name, 1));
        if (\is_array($value)) {
            $value = \count($value);
        }
        return $value;
    ...
    protected function delete_old_meta($to_id, $meta_to_copy)
    {
        ...
        $delete_query = $wpdb->prepare("DELETE FROM `{$wpdb->postmeta}` WHERE `meta_key` IN ({$keys}) AND `post_id` = %d", $to_id);
