...
use Whisk\Recipes\Vendor\Carbon_Fields\Container\Container;
use Whisk\Recipes\Vendor\Carbon_Fields\Container\Repository as ContainerRepository;
use Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set;
use Whisk\Recipes\Vendor\Carbon_Fields\Toolset\Key_Toolset;
use Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface;
/*
 * Service which provides the ability to do meta queries for multi-value fields and nested fields
 */
class Legacy_Storage_Service_v_1_5 extends \Whisk\Recipes\Vendor\Carbon_Fields\Service\Service
{
    /**
     ...
     */
    public function __construct(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Repository $container_repository, \Whisk\Recipes\Vendor\Carbon_Fields\Toolset\Key_Toolset $key_toolset)
    {
     ...
     * @return Container
     */
    protected function get_container_for_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
    ...
    protected function get_field_group_permutations($fields)
    {
        ...
        foreach ($fields as $field) {
            if ($field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Complex_Field) {
                ...
                foreach ($group_names as $group_name) {
                    ...
                    $permutations[] = array('field' => $field->get_base_name(), 'group' => $group_name, 'children' => $this->get_field_group_permutations($group->get_fields()));
                }
            } else {
                $permutations[] = array('field' => $field->get_base_name(), 'group' => '', 'children' => array());
            }
        }
        return $permutations;
    }
     ...
     * @return array
     */
    protected function get_table_details_for_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
        global $wpdb;
        $details = array('prefix' => '_', 'table_name' => '', 'table_id_column' => '', 'table_key_column' => '', 'table_value_column' => '');
        if ($datastore instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Theme_Options_Datastore) {
            $details['prefix'] = '';
            $details['table_name'] = $wpdb->options;
            $details['table_key_column'] = 'option_name';
            $details['table_value_column'] = 'option_value';
        } else {
            if ($datastore instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Meta_Datastore) {
     ...
     * @return array<string>
     */
    protected function get_legacy_sql_comparisons_for_field(\Whisk\Recipes\Vendor\Carbon_Fields\Field\Field $field, $key_prefix, $key_column)
    {
        ...
        if ($field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Complex_Field) {
            ...
            foreach ($groups as $group_name) {
                $underscored_group_name = \preg_replace('/^_{0,1}/', '_', $group_name);
                // ensure first character is underscore
        ...
        if ($field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Map_Field) {
     ...
     * @return array
     */
    protected function get_legacy_storage_array_from_database(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Container $container, \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
        global $wpdb;
        $table = $this->get_table_details_for_datastore($datastore);
        if ($table['table_id_column'] && !$datastore->get_object_id()) {
            return array();
            // bail as we have an ID column but no ID to compare with ( e.g. we are in a "create" view )
        }
        $comparisons = array();
     ...
     * @return array
     */
    public function get_legacy_storage_array(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
        $container = $this->get_container_for_datastore($datastore);
        if (!$container) {
            return array();
            // unhandled datastore type or no registered containers
        }
    ...
    protected function get_value_for_legacy_key($key, $legacy_storage_array)
    {
        ...
        if (isset($legacy_storage_array[$key . '-' . $first_map_key])) {
            $value = array(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $value);
            foreach ($this->map_keys as $map_key) {
    ...
    protected function legacy_storage_rows_to_row_descriptors($legacy_storage_array, $field_group_permutations)
    {
        ...
        foreach ($legacy_storage_array as $key => $value) {
            if ($this->is_legacy_map_key($key)) {
                continue;
                // skip legacy map keys as they are handled separately through values
            }
        ...
        foreach ($key_pieces as $piece) {
            foreach ($field_group_level as $permutation) {
                ...
                if (\preg_match($match_regex, $piece, $matches)) {
                    $match_data = array('field' => $permutation['field'], 'group' => $permutation['group'], 'group_index' => isset($matches['group_index']) ? \intval($matches['group_index']) : -1);
                    $previous_match_index = \count($matched_fields) - 1;
                    if (isset($matched_fields[$previous_match_index])) {
                        $matched_fields[$previous_match_index]['group_index'] = $match_data['group_index'];
                        // indexes are offset in CF 1.5 complex keys
                        $match_data['group_index'] = 0;
                    }
                    $matched_fields[] = $match_data;
                    $field_group_level = $permutation['children'];
                    break;
                    // break so next piece foreaches the new field_group_level
                }
            }
        }
        return array('key' => $matched_fields, 'value' => $value);
    }
    /**
    ...
    protected function row_descriptor_to_storage_array($row_descriptor)
    {
        ...
        }, \array_slice($row_descriptor['key'], 0, -1));
        // last index is not part of the hierarchy index
        ...
        foreach ($complex_parents as $level => $complex_parent) {
            ...
            $key = $this->key_toolset->get_storage_key(\false, \array_slice($hierarchy, 0, $level + 1), $complex_parent_hierarchy_index, $complex_parent_value_index, \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
        ...
        if (\is_array($unserialized_value)) {
            if (isset($unserialized_value[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY])) {
                ...
                foreach ($unserialized_value as $value_item) {
                    $key = $this->key_toolset->get_storage_key(\false, $hierarchy, $hierarchy_index, $i, \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
                    $storage_array[$key] = $value_item;
                    $i++;
                }
            }
            ...
            if ($unserialized_value === null) {
                ...
                $key = $this->key_toolset->get_storage_key(\false, $hierarchy, $hierarchy_index, 0, \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
                $storage_array[$key] = $row_descriptor['value'];
            }
        }
     ...
     * @return array
     */
    public function get_storage_array_for_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
     ...
     * @return array
     */
    public function get_storage_array(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore, $storage_key_patterns)
    {
        ...
        foreach ($storage_array as $key => $value) {
            if ($this->key_toolset->storage_key_matches_any_pattern($key, $storage_key_patterns)) {
                $matched_data[] = (object) array('key' => $key, 'value' => $value);
            }
        }
        return $matched_data;
    }
