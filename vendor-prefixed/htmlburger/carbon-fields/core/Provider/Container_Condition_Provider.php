...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Provider;

use Whisk\Recipes\Vendor\Carbon_Fields\Pimple\Container as PimpleContainer;
use Whisk\Recipes\Vendor\Carbon_Fields\Pimple\ServiceProviderInterface;
use Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Factory as ConditionFactory;
use Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection;
class Container_Condition_Provider implements \Whisk\Recipes\Vendor\Carbon_Fields\Pimple\ServiceProviderInterface
{
    /**
     ...
     */
    public function register(\Whisk\Recipes\Vendor\Carbon_Fields\Pimple\Container $ioc)
    {
    ...
    protected function install_conditions($ioc)
    {
        $ioc['container_condition_factory'] = function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Factory($ioc['container_conditions']);
        };
        $ioc['container_condition_fulfillable_collection'] = $ioc->factory(function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection($ioc['container_condition_factory'], $ioc['container_condition_translator_array']);
        });
        $ioc['container_conditions'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Pimple\Container();
        };
        $cc_ioc = $ioc['container_conditions'];
        $cc_ioc['boolean'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Boolean_Condition();
            $condition->set_comparers(array($ioc['container_condition_comparers']['equality']));
            return $condition;
        });
        $cc_ioc['post_id'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_ID_Condition();
        ...
        $cc_ioc['post_parent_id'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Parent_ID_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['post_ancestor_id'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Ancestor_ID_Condition();
        ...
        $cc_ioc['post_type'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Type_Condition();
        ...
        $cc_ioc['post_format'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Format_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['nonscalar']);
            return $condition;
        });
        $cc_ioc['post_level'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Level_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['post_template'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Template_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['nonscalar']);
            return $condition;
        });
        $cc_ioc['post_term'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Post_Term_Condition($ioc['wp_toolset']);
        ...
        $cc_ioc['term'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Term_Condition($ioc['wp_toolset']);
            $condition->set_comparers(array(
                // Only support the custom comparer as this condition has its own comparison methods
                $ioc['container_condition_comparers']['custom'],
            ));
        ...
        $cc_ioc['term_taxonomy'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Term_Taxonomy_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['nonscalar']);
            return $condition;
        });
        $cc_ioc['term_level'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Term_Level_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['term_parent'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Term_Parent_Condition($ioc['wp_toolset']);
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['term_ancestor'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Term_Ancestor_Condition($ioc['wp_toolset']);
            $condition->set_comparers($ioc['container_condition_comparer_collections']['array']);
            return $condition;
        });
        $cc_ioc['user_id'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\User_ID_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['user_role'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\User_Role_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['array']);
            return $condition;
        });
        $cc_ioc['user_capability'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\User_Capability_Condition();
            $condition->set_comparers(array(
                // Only support the custom comparer as this condition has its own comparison methods
                $ioc['container_condition_comparers']['custom'],
            ));
        ...
        $cc_ioc['blog_id'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Blog_ID_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['current_user_id'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Current_User_ID_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['generic']);
            return $condition;
        });
        $cc_ioc['current_user_role'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Current_User_Role_Condition();
            $condition->set_comparers($ioc['container_condition_comparer_collections']['array']);
            return $condition;
        });
        $cc_ioc['current_user_capability'] = $cc_ioc->factory(function () use($ioc) {
            $condition = new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Current_User_Capability_Condition();
            $condition->set_comparers(array(
                // Only support the custom comparer as this condition has its own comparison methods
                $ioc['container_condition_comparers']['custom'],
            ));
    ...
    protected function install_comparers($ioc)
    {
        ...
        $ioc['container_condition_comparers']['equality'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Comparer\Equality_Comparer();
        };
        $ioc['container_condition_comparers']['any_equality'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Comparer\Any_Equality_Comparer();
        };
        $ioc['container_condition_comparers']['contain'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Comparer\Contain_Comparer();
        };
        $ioc['container_condition_comparers']['any_contain'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Comparer\Any_Contain_Comparer();
        };
        $ioc['container_condition_comparers']['scalar'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Comparer\Scalar_Comparer();
        };
        $ioc['container_condition_comparers']['custom'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Comparer\Custom_Comparer();
        };
        ...
        $ioc['container_condition_comparer_collections']['generic'] = function () use($ioc) {
            return array($ioc['container_condition_comparers']['equality'], $ioc['container_condition_comparers']['contain'], $ioc['container_condition_comparers']['scalar'], $ioc['container_condition_comparers']['custom']);
        };
        $ioc['container_condition_comparer_collections']['nonscalar'] = function () use($ioc) {
            return array($ioc['container_condition_comparers']['equality'], $ioc['container_condition_comparers']['contain'], $ioc['container_condition_comparers']['custom']);
        };
        $ioc['container_condition_comparer_collections']['array'] = function () use($ioc) {
            return array($ioc['container_condition_comparers']['any_equality'], $ioc['container_condition_comparers']['any_contain'], $ioc['container_condition_comparers']['custom']);
        };
    }
    /**
    ...
    protected static function install_translators($ioc)
    {
        $ioc['container_condition_translator_array'] = function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Translator\Array_Translator($ioc['container_condition_factory']);
        };
        $ioc['container_condition_translator_json'] = function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Translator\Json_Translator($ioc['container_condition_factory']);
        };
    }
    /**
    ...
    protected function install_container_conditions($ioc)
    {
        ...
        add_filter('carbon_fields_container_static_condition_types', function ($condition_types, $container_type, $container) {
            return \array_merge($condition_types, array('current_user_id', 'current_user_role', 'current_user_capability'));
        }, 10, 3);
    ...
    {
        return \array_merge($condition_types, array('post_id', 'post_type'));
    }
    /**
    ...
    {
        return \array_merge($condition_types, array('post_parent_id', 'post_ancestor_id', 'post_format', 'post_level', 'post_template', 'post_term'));
    }
    /**
    ...
    {
        return \array_merge($condition_types, array('term', 'term_taxonomy'));
    }
    /**
    ...
    {
        return \array_merge($condition_types, array('term_level', 'term_parent', 'term_ancestor'));
    }
    /**
    ...
    {
        return \array_merge($condition_types, array('user_id', 'user_capability'));
    }
    /**
    ...
    {
        return \array_merge($condition_types, array('user_role'));
    }
    /**
    ...
    {
        return \array_merge($condition_types, array('blog_id'));
    }
}
