...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Toolset;
...
class Key_Toolset
{
    ...
    protected function get_storage_key_prefix($full_hierarchy, $full_hierarchy_index)
    {
        ...
        $storage_key = static::KEY_PREFIX . $first_parent . static::SEGMENT_GLUE . \implode(static::SEGMENT_VALUE_GLUE, $parents) . static::SEGMENT_GLUE . \implode(static::SEGMENT_VALUE_GLUE, $full_hierarchy_index) . static::SEGMENT_GLUE;
        return $storage_key;
    }
    /**
    ...
    {
        $full_hierarchy_index = $this->get_sanitized_hierarchy_index($full_hierarchy, $full_hierarchy_index);
        if ($is_simple_root_field && $property === \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY) {
     ...
     * @return string
     */
    public function get_storage_key_with_index_wildcards($is_simple_root_field, $full_hierarchy, $property = \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY, $wildcard = '%')
    {
        if ($is_simple_root_field && $property === \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY) {
            return $this->get_storage_key_for_simple_root_field(\array_shift($full_hierarchy));
        }
        ...
        $storage_key = static::KEY_PREFIX . $first_parent . static::SEGMENT_GLUE . \implode(static::SEGMENT_VALUE_GLUE, $parents) . static::SEGMENT_GLUE . $hierarchy_index . static::SEGMENT_GLUE . $value_group_index . static::SEGMENT_GLUE . $property;
        return $storage_key;
    }
    /**
        ...
        foreach ($patterns as $storage_key => $type) {
            ...
            switch ($type) {
                ...
                case static::PATTERN_COMPARISON_STARTS_WITH:
                    ...
                    break;
                default:
                    \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unsupported storage key pattern type used: "' . $type . '"');
                    break;
            }
            $sql[] = $comparison;
        }
    ...
    {
        $key = \substr($storage_key, \strlen(static::KEY_PREFIX));
        // drop the key prefix
    ...
    {
        $parsed = array('root' => '', 'hierarchy' => array(), 'hierarchy_index' => array(), 'value_index' => 0, 'property' => \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
