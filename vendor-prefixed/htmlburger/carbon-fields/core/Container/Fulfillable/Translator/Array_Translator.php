...
class Array_Translator extends \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Translator\Translator
{
    /**
     ...
     */
    public function __construct(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Factory $condition_factory)
    {
     ...
     * {@inheritDoc}
     */
    protected function condition_to_foreign(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Condition $condition)
    {
        return array('type' => $this->condition_factory->get_type(\get_class($condition)), 'compare' => $condition->get_comparison_operator(), 'value' => $condition->get_value());
    }
    /**
     * {@inheritDoc}
     */
    protected function fulfillable_collection_to_foreign(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection $fulfillable_collection)
    {
        $fulfillables = $fulfillable_collection->get_fulfillables();
        if (empty($fulfillables)) {
            return array();
        }
        $collection = array('relation' => 'AND');
        $relations = array();
    ...
    {
        if (!\is_array($foreign)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid data passed to array condition translator: ' . \print_r($foreign, \true));
            return null;
        }
    ...
    protected function foreign_to_native_fulfillable_collection($foreign)
    {
        ...
        $collection = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('container_condition_fulfillable_collection');
        foreach ($foreign as $key => $value) {
            if ($key === 'relation') {
                continue;
                // ignore the relation key - we are only interested in condition definitions
            }
