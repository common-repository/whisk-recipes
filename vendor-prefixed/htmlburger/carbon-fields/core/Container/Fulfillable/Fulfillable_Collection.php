...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable;

use Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Factory;
use Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Translator\Array_Translator;
use Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception;
class Fulfillable_Collection implements \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable
{
    /**
     ...
     */
    public function __construct(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Condition\Factory $condition_factory, \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Translator\Array_Translator $array_translator)
    {
    ...
    {
        return \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('container_condition_fulfillable_collection');
    }
    /**
    ...
    protected function propagate_condition_type_list()
    {
        ...
        foreach ($fulfillables as $fulfillable) {
            if ($fulfillable['fulfillable'] instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection) {
    ...
    public function where($condition_type, $comparison_operator = '=', $value = null, $fulfillable_comparison = 'AND')
    {
        ...
        if (!$this->is_condition_type_allowed($condition_type)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unsupported container condition used: ' . $condition_type);
            return $this;
        }
        if ($value === null) {
     ...
     */
    public function add_fulfillable(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable $fulfillable, $fulfillable_comparison)
    {
        if (!\in_array($fulfillable_comparison, $this->supported_fulfillable_comparisons)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid fulfillable comparison passed: ' . $fulfillable_comparison);
            return;
        }
        $this->fulfillables[] = array('fulfillable_comparison' => $fulfillable_comparison, 'fulfillable' => $fulfillable);
    }
    /**
     ...
     */
    public function remove_fulfillable(\Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable $fulfillable)
    {
        $fulfillables = $this->get_fulfillables();
        foreach ($fulfillables as $index => $fulfillable_tuple) {
            if ($fulfillable_tuple['fulfillable'] === $fulfillable) {
                $fulfillables_copy = $fulfillables;
                // introduce a copy array to highlight array_splice mutation
                \array_splice($fulfillables_copy, $index, 1);
                $this->fulfillables = \array_values($fulfillables_copy);
                // make sure our array is indexed cleanly
                return \true;
            }
        }
        return \false;
        ...
        foreach ($fulfillables as $fulfillable_tuple) {
            ...
            if ($fulfillable instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection) {
                ...
                if (empty($filtered_collection_fulfillables)) {
                    continue;
                    // skip empty collections to reduce clutter
                }
    ...
    {
        $fulfilled = \true;
        // return true for empty collections
        $fulfillables = $this->get_fulfillables();
