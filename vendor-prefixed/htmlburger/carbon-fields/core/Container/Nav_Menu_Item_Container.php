 ...
 */
class Nav_Menu_Item_Container extends \Whisk\Recipes\Vendor\Carbon_Fields\Container\Container
{
    /**
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        if (!$this->get_datastore()) {
            $this->set_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore::make('nav_menu_item'), $this->has_default_datastore());
        }
    ...
    {
        include \Whisk\Recipes\Vendor\Carbon_Fields\DIR . '/templates/Container/nav_menu_item.php';
    }
    /**
     ...
     */
    protected function get_clone_for_menu_item($menu_item_id, $load = \true)
    {
        if (!isset($this->menu_item_instances[$menu_item_id])) {
            $menu_item_datastore = \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore::make('nav_menu_item');
            ...
            foreach ($fields as $field) {
                ...
                $tmp_field->set_datastore($menu_item_datastore, \true);
                $custom_fields[] = $tmp_field;
            }
            $container = \Whisk\Recipes\Vendor\Carbon_Fields\Container\Container::factory($this->type, $menu_item_field_prefix . $this->get_id())->set_datastore($menu_item_datastore, \true)->add_fields($custom_fields)->init($menu_item_id);
            if ($load) {
                $container->load();
            }
    ...
    {
        return 'Whisk\\Recipes\\Vendor\\Whisk\\Recipes\\Vendor\\Carbon_Fields\\Walker\\Nav_Menu_Item_Edit_Walker';
    }
}
