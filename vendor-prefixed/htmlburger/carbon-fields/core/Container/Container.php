...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Container;
...
use Whisk\Recipes\Vendor\Carbon_Fields\Field\Group_Field;
 ...
 */
abstract class Container implements \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Holder_Interface
{
    /**
    ...
    public static function factory($raw_type, $id, $name = '')
    {
        ...
        $repository = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('container_repository');
        $id = $repository->get_unique_container_id($id !== '' ? $id : $name);
        if (!\Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::is_valid_entity_id($id)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Container IDs can only contain lowercase alphanumeric characters, dashes and underscores ("' . $id . '" passed).');
            return null;
        }
        if (!$repository->is_unique_container_id($id)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('The passed container id is already taken ("' . $id . '" passed).');
            return null;
        }
        $container = null;
        if (\Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::has($type, 'containers')) {
            $container = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve_with_arguments($type, array('id' => $id, 'name' => $name, 'type' => $type), 'containers');
        } else {
            // Fallback to class name-based resolution
            $class = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::type_to_class($type, __NAMESPACE__, '_Container');
            if (!\class_exists($class)) {
                \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unknown container "' . $raw_type . '".');
                $class = __NAMESPACE__ . '\\Broken_Container';
            }
            $fulfillable_collection = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('container_condition_fulfillable_collection');
            $condition_translator = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('container_condition_translator_json');
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        if (empty($title)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Empty container title is not supported');
        }
        $this->id = $id;
        $this->title = $title;
        $this->type = $type;
    ...
    protected function get_condition_types($static)
    {
        ...
        $container_type = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::class_to_type(\get_class($this), '_Container');
    ...
    {
        foreach ($this->fields as $field) {
            $field->set_value_from_input(\Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::input());
            $field->save();
        }
    }
    /**
    ...
    public function get_field_by_name($field_name)
    {
        ...
        while (!empty($hierarchy_left)) {
            ...
            $segment_group_name = isset($segment_pieces['group_name']) ? $segment_pieces['group_name'] : \Whisk\Recipes\Vendor\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
            foreach ($field_group as $f) {
                ...
                if (empty($hierarchy_left)) {
                    ...
                    if (!$f instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Complex_Field) {
                        return null;
                    }
    ...
    protected function register_field_name($name)
    {
        if (\in_array($name, $this->registered_field_names)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Field name "' . $name . '" already registered');
            return \false;
        }
    ...
    {
        $input = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::input();
    ...
    private function create_tab($tab_name, $fields, $queue_end = self::TABS_TAIL)
    {
        if (isset($this->tabs[$tab_name])) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise("Tab name duplication for {$tab_name}");
        }
    ...
    public function add_fields($fields)
    {
        foreach ($fields as $field) {
            if (!$field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field) {
                \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Whisk\\Recipes\\Vendor\\Object must be of type Whisk\\Recipes\\Vendor\\Carbon_Fields\\Field\\Field');
                return $this;
            }
