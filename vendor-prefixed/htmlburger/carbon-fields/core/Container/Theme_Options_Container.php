...
use Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore;
use Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception;
/**
 * Theme options container class.
 */
class Theme_Options_Container extends \Whisk\Recipes\Vendor\Carbon_Fields\Container\Container
{
    /**
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        if (!$this->get_datastore()) {
            $this->set_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore::make('theme_options'), $this->has_default_datastore());
        }
        if (apply_filters('carbon_fields_' . $type . '_container_admin_only_access', \true, $title, $this)) {
    ...
    protected function title_to_filename($title, $extension)
    {
        ...
        $title = \preg_replace(array('~\\s+~', '~[^\\w\\d-]+~u', '~-+~'), array('-', '-', '-'), $title);
    ...
    public function attach()
    {
        ...
        if (!$this->settings['parent']) {
            add_menu_page($this->title, $this->settings['menu_title'] ? $this->settings['menu_title'] : $this->title, 'read', $this->get_page_file(), array($this, 'render'), $this->settings['icon'], $this->settings['menu_position']);
        }
        add_submenu_page($this->settings['parent'], $this->title, $this->settings['menu_title'] ? $this->settings['menu_title'] : $this->title, 'read', $this->get_page_file(), array($this, 'render'));
    ...
    public function render()
    {
        ...
        include \Whisk\Recipes\Vendor\Carbon_Fields\DIR . '/templates/Container/' . $this->type . '.php';
    }
    /**
    ...
    protected function register_page()
    {
        ...
        if (!$parent) {
            ...
            if (isset(static::$registered_pages[$file])) {
                \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Page "' . $file . '" already registered');
                return \false;
            }
        ...
        if (\in_array($file, static::$registered_pages[$parent])) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Page "' . $file . '" with parent "' . $parent . '" is already registered. Please set a name for the container.');
            return \false;
        }
