...
use Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Meta_Datastore;
use Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception;
/**
 * Comment meta container class.
 */
class Comment_Meta_Container extends \Whisk\Recipes\Vendor\Carbon_Fields\Container\Container
{
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        if (!$this->get_datastore()) {
            $this->set_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore::make('comment_meta'), $this->has_default_datastore());
        }
    }
    /**
    ...
    {
        if (isset($_GET['c']) && ($comment_id = absint($_GET['c']))) {
            // Input var okay.
    ...
    protected function get_environment_for_request()
    {
        ...
        $environment = array('comment_id' => isset($input['c']) ? \intval($input['c']) : 0);
        return $environment;
    }
    /**
    ...
    {
        $environment = array('comment_id' => \intval($object_id));
        return $environment;
    }
    /**
    ...
    {
        add_meta_box($this->get_id(), $this->title, array($this, 'render'), 'comment', 'normal', 'high');
    }
    /**
    ...
    {
        include \Whisk\Recipes\Vendor\Carbon_Fields\DIR . '/templates/Container/comment_meta.php';
    }
    /**
