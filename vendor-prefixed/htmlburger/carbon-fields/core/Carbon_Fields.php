...
namespace Whisk\Recipes\Vendor\Carbon_Fields;
...
use Whisk\Recipes\Vendor\Carbon_Fields\Loader\Loader;
...
use Whisk\Recipes\Vendor\Carbon_Fields\Toolset\WP_Toolset;
use Whisk\Recipes\Vendor\Carbon_Fields\Service\Meta_Query_Service;
use Whisk\Recipes\Vendor\Carbon_Fields\Service\Legacy_Storage_Service_v_1_5;
use Whisk\Recipes\Vendor\Carbon_Fields\Service\Revisions_Service;
use Whisk\Recipes\Vendor\Carbon_Fields\Service\REST_API_Service;
use Whisk\Recipes\Vendor\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager;
use Whisk\Recipes\Vendor\Carbon_Fields\REST_API\Router as REST_API_Router;
use Whisk\Recipes\Vendor\Carbon_Fields\REST_API\Decorator as REST_API_Decorator;
use Whisk\Recipes\Vendor\Carbon_Fields\Event\Emitter;
use Whisk\Recipes\Vendor\Carbon_Fields\Event\PersistentListener;
use Whisk\Recipes\Vendor\Carbon_Fields\Event\SingleEventListener;
...
final class Carbon_Fields
{
    ...
    public static function resolve_with_arguments($identifier, $arguments, $subcontainer = null)
    {
        ...
        $container = new \Whisk\Recipes\Vendor\Carbon_Fields\Pimple\Container();
    ...
    {
        $type_dictionary = array('_Container' => 'containers', '_Field' => 'fields', '_Condition' => 'container_conditions');
        $extension_suffix = '';
        ...
        if (empty($extension_suffix)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Could not determine "' . $class . '" extension type. Extension classes must have one of the following suffixes: ' . \implode(', ', \array_keys($type_dictionary)));
            return;
        }
        $identifier = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::class_to_type($class, $extension_suffix);
     ...
     */
    public function install(\Whisk\Recipes\Vendor\Carbon_Fields\Pimple\Container $ioc)
    {
        $this->ioc = $ioc;
    }
    /**
    ...
    public static function boot()
    {
        ...
        if (\defined(__NAMESPACE__ . '\\VERSION')) {
            return;
            // Possibly attempting to load multiple versions of Carbon Fields; bail in favor of already loaded version
        }
        static::resolve('loader')->boot();
        static::instance()->booted = \true;
    ...
    {
        if (!static::is_booted()) {
            throw new \Exception('You must call Whisk\\Recipes\\Vendor\\Carbon_Fields\\Carbon_Fields::boot() in a suitable WordPress hook before using Carbon Fields.');
        }
    }
    /**
    ...
    public static function directory_to_url($directory)
    {
        ...
        return '';
        // return empty string to avoid exposing half-parsed paths
    }
    /**
    ...
    {
        $ioc = new \Whisk\Recipes\Vendor\Carbon_Fields\Pimple\Container();
        $ioc['loader'] = function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Loader\Loader($ioc['sidebar_manager'], $ioc['container_repository']);
        };
        $ioc['container_repository'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Container\Repository();
        };
        ...
        $ioc['key_toolset'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Toolset\Key_Toolset();
        };
        $ioc['wp_toolset'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Toolset\WP_Toolset();
        };
        $ioc['sidebar_manager'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager();
        };
        $ioc['rest_api_router'] = function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\REST_API\Router($ioc['container_repository']);
        };
        $ioc['rest_api_decorator'] = function ($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\REST_API\Decorator($ioc['container_repository']);
        };
        /* Services */
        ...
        $ioc['services']['revisions'] = function () use($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Service\Revisions_Service();
        };
        $ioc['services']['meta_query'] = function () use($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Service\Meta_Query_Service($ioc['container_repository'], $ioc['key_toolset']);
        };
        $ioc['services']['legacy_storage'] = function () use($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Service\Legacy_Storage_Service_v_1_5($ioc['container_repository'], $ioc['key_toolset']);
        };
        $ioc['services']['rest_api'] = function () use($ioc) {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Service\REST_API_Service($ioc['rest_api_router'], $ioc['rest_api_decorator']);
        };
        /* Events */
        $ioc['event_emitter'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Event\Emitter();
        };
        $ioc['event_persistent_listener'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Event\PersistentListener();
        };
        $ioc['event_single_event_listener'] = function () {
            return new \Whisk\Recipes\Vendor\Carbon_Fields\Event\SingleEventListener();
        };
        $ioc->register(new \Whisk\Recipes\Vendor\Carbon_Fields\Provider\Container_Condition_Provider());
        return $ioc;
    }
}
