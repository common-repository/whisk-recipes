...
namespace Whisk\Recipes\Vendor\Carbon_Fields\Value_Set;
...
class Value_Set
{
    ...
    public function __construct($type = self::TYPE_SINGLE_VALUE, $additional_properties = array())
    {
        if (!\in_array($type, $this->valid_types)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise("Invalid type specified for Value_Set: {$type}");
        }
        $this->type = $type;
        ...
        if (empty($string_keys)) {
            ...
            foreach ($value_array as $key => $value) {
                if (!\is_array($value) || !isset($value[static::VALUE_PROPERTY])) {
                    ...
                    $value = array(static::VALUE_PROPERTY => $value);
                }
                $raw_value_set[] = $value;
            }
        } else {
