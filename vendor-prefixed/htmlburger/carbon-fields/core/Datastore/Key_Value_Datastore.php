 ...
 */
abstract class Key_Value_Datastore extends \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore
{
    /**
    ...
    {
        $this->key_toolset = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('key_toolset');
        parent::__construct();
    }
    /**
     ...
     * @return array<string>
     */
    protected function get_full_hierarchy_for_field(\Whisk\Recipes\Vendor\Carbon_Fields\Field\Field $field)
    {
     ...
     * @return array<int>
     */
    protected function get_full_hierarchy_index_for_field(\Whisk\Recipes\Vendor\Carbon_Fields\Field\Field $field)
    {
    ...
    protected function cascading_storage_array_to_value_tree_array($storage_array)
    {
        ...
        \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::ksort_recursive($tree);
     ...
     * @return array
     */
    protected function value_tree_array_to_value_tree($value_tree_array, \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field $field)
    {
     ...
     */
    protected abstract function get_storage_array(\Whisk\Recipes\Vendor\Carbon_Fields\Field\Field $field, $storage_key_patterns);
    /**
    ...
    public function save(\Whisk\Recipes\Vendor\Carbon_Fields\Field\Field $field)
    {
        ...
        if (empty($value_set) && $field->get_value_set()->keepalive()) {
            $storage_key = $this->key_toolset->get_storage_key($field->is_simple_root_field(), $this->get_full_hierarchy_for_field($field), $this->get_full_hierarchy_index_for_field($field), 0, $this->key_toolset->get_keepalive_property());
            $this->save_key_value_pair($storage_key, '');
        }
        foreach ($value_set as $value_group_index => $values) {
            foreach ($values as $property => $value) {
                $storage_key = $this->key_toolset->get_storage_key($field->is_simple_root_field(), $this->get_full_hierarchy_for_field($field), $this->get_full_hierarchy_index_for_field($field), $value_group_index, $property);
