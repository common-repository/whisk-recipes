 ...
 */
abstract class Datastore implements \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface
{
    /**
    ...
    public static function factory($raw_type)
    {
        ...
        $class = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::type_to_class($type, __NAMESPACE__, '_Datastore');
        if (!\class_exists($class)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unknown datastore type "' . $raw_type . '".');
            return null;
        }
        $datastore = new $class();
        return $datastore;
