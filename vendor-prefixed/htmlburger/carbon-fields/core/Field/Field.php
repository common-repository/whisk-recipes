...
use Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Holder_Interface;
 ...
 */
class Field implements \Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Holder_Interface
{
    /**
    ...
    {
        $type = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::normalize_type($raw_type);
        // stop hidden symbol support when the end user is creating fields ][
        // @see Field::set_name()
        if (!\Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::is_valid_entity_id($name)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Field names can only contain lowercase alphanumeric characters, dashes and underscores ("' . $name . '" passed).');
            return null;
        }
        if (\Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::has($type, 'fields')) {
            return \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve_with_arguments($type, array('type' => $type, 'name' => $name, 'label' => $label), 'fields');
        }
        // Fallback to class name-based resolution
        $class = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::type_to_class($type, __NAMESPACE__, '_Field');
        if (!\class_exists($class)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unknown field type "' . $raw_type . '".');
    ...
    {
        \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::verify_boot();
        $this->type = $type;
    ...
    {
        return \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::class_to_type(\get_class($this), '_Field');
    }
    /**
    ...
    public function is_simple_root_field()
    {
        ...
        return empty($hierarchy) && \in_array($this->get_value_set()->get_type(), array(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::TYPE_SINGLE_VALUE, \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::TYPE_MULTIPLE_PROPERTIES));
    }
    /**
     ...
     * @return mixed
     */
    protected function get_value_from_datastore($fallback_to_default = \true)
    {
    ...
    {
        $delete_on_save = apply_filters('carbon_fields_should_delete_field_value_on_save', \true, $this);
        if ($delete_on_save) {
            $this->delete();
        }
    ...
    public function get_value_set()
    {
        if ($this->value_set === null) {
            $this->set_value_set(new \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set());
        }
    ...
    {
        if (empty($name)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Field name can\'t be empty');
            return $this;
        }
        // symbols ][ are supported in a hidden way - required for widgets to work (WP imposes dashes and square brackets on field names)
        $field_name_characters = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::get_field_name_characters_pattern();
    ...
    public function set_label($label)
    {
        if (\is_null($label)) {
            ...
            $label = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::normalize_label($this->get_name());
        }
        $this->label = $label;
        return $this;
    }
    ...
    public function set_attribute($name, $value = '')
    {
        ...
        if (!$is_data_attribute && !\in_array($name, $this->allowed_attributes)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Only the following attributes are allowed: ' . \implode(', ', \array_merge($this->allowed_attributes, array('data-*'))));
            return $this;
        }
    ...
    public function set_attributes($attributes)
    {
        if (!\is_array($attributes)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('An array must be passed for the $attributes parameter of Field::set_attributes().');
            return $this;
        }
    ...
    {
        $this->classes = \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::sanitize_classes($classes);
        return $this;
    }
    /**
     ...
     * @return self  $this
     */
    public function set_required($required = \true)
    {
    ...
    protected function parse_conditional_rule($rule)
    {
        ...
        if (!\is_array($rule) || empty($rule['field'])) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid conditional logic rule format. The rule should be an array with the "field" key set.');
            return null;
        }
        // Fill in optional keys with defaults
        $rule = \array_merge(array('compare' => '=', 'value' => ''), $rule);
        if (!\in_array($rule['compare'], $allowed_operators)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid conditional logic compare operator: <code>' . $rule['compare'] . '</code><br>Allowed operators are: <code>' . \implode(', ', $allowed_operators) . '</code>');
            return null;
        }
        if (\in_array($rule['compare'], $array_operators) && !\is_array($rule['value'])) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid conditional logic value format. An array is expected, when using the "' . $rule['compare'] . '" operator.');
            return null;
        }
        return $rule;
    }
    ...
    {
        if (!\is_array($rules)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Conditional logic rules argument should be an array.');
            return array();
        }
        $parsed_rules = array('relation' => \Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::get_relation_type_from_array($rules), 'rules' => array());
        $rules_only = $rules;
        unset($rules_only['relation']);
        // Skip the relation key as it is already handled above
     ...
     * @return self  $this
     */
    public function set_visible_in_rest_api($visible = \true)
    {
