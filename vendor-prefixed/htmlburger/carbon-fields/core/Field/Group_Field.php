...
class Group_Field
{
    ...
    public function add_fields($fields)
    {
        foreach ($fields as $field) {
            if (!$field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field) {
                \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Object must be of type ' . __NAMESPACE__ . '\\Field');
            }
    ...
    public function to_json($load)
    {
        ...
        $group_data = array('group_id' => $this->get_group_id(), 'name' => $this->get_name(), 'label' => $this->get_label(), 'label_template' => $this->get_label_template(), 'fields' => $fields_data);
        return $group_data;
    }
    /**
    ...
    public function set_name($name)
    {
        ...
        if (!\Whisk\Recipes\Vendor\Carbon_Fields\Helper\Helper::is_valid_entity_id($name)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Group names can only contain lowercase alphanumeric characters, dashes and underscores ("' . $name . '" passed).');
            return $this;
        }
        $this->name = $name;
        return $this;
