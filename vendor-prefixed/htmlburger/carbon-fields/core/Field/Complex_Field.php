 ...
 */
class Complex_Field extends \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field
{
    /**
     ...
     * @var boolean
     */
    protected $duplicate_groups_allowed = \true;
    /**
     * Entry labels
    ...
    {
        $this->set_value_set(new \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::TYPE_MULTIPLE_VALUES));
    ...
    {
        $this->labels = array('singular_name' => __($this->labels['singular_name'], 'carbon-fields'), 'plural_name' => __($this->labels['plural_name'], 'carbon-fields'));
        parent::init();
    }
    /**
     ...
     * @return self                $this
     */
    public function set_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore, $set_as_default = \false)
    {
        ...
        $this->update_child_datastore($this->get_datastore(), \true);
        return $this;
    }
    /**
     ...
     * @param boolean             $set_as_default
     */
    protected function update_child_datastore(\Whisk\Recipes\Vendor\Carbon_Fields\Datastore\Datastore_Interface $datastore, $set_as_default = \false)
    {
    ...
    public function add_fields()
    {
        ...
        $name = !empty($name) ? $name : \Whisk\Recipes\Vendor\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
        if (\array_key_exists($name, $this->groups)) {
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Group with name "' . $name . '" in Complex Field "' . $this->get_label() . '" already exists.');
            return $this;
        }
        $reserved_names = array(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY, static::TYPE_PROPERTY);
        foreach ($fields as $field) {
            /** @var Field $field */
            if (\in_array($field->get_base_name(), $reserved_names)) {
                \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('"' . $field->get_base_name() . '" is a reserved keyword for Complex fields and cannot be used for a field name.');
                return $this;
            }
        }
        $group = new \Whisk\Recipes\Vendor\Carbon_Fields\Field\Group_Field($name, $label, $fields);
    ...
    protected function get_prefilled_groups($groups, $value_tree)
    {
        ...
        foreach ($value_tree as $group_index => $value) {
            ...
            $fields[$group_index] = array(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $group->get_name()) + $this->get_prefilled_group_fields($group_fields, $group_values, $group_index);
        }
        return $fields;
    }
    /**
    ...
    public function set_value_from_input($input)
    {
        ...
        foreach ($input_groups as $values) {
            if (!isset($values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]) || !isset($this->groups[$values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]])) {
                continue;
            }
            $group = $this->get_group_by_name($values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
            $group_fields = $group->get_fields();
            $group_field_names = \array_flip($group->get_field_names());
            $value_group = array(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
            unset($values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
            ...
            foreach ($group_fields as $field) {
                ...
                if ($tmp_field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Complex_Field) {
    ...
    public function save()
    {
        ...
        if ($save) {
            ...
            foreach ($field_groups as $group_index => $fields) {
                foreach ($fields as $field) {
                    if (!$field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field) {
                        continue;
                    }
                    $field->save();
                }
        ...
        foreach ($field_groups as $group_index => $field_group) {
            ...
            foreach ($field_group as $key => $field) {
                if ($field instanceof \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field) {
                    ...
                    if ($key === \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY) {
    ...
    protected function external_to_internal_value($value)
    {
        ...
        $value[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY] = $value[static::TYPE_PROPERTY];
    ...
    public function set_value($value)
    {
        ...
        foreach ($value as $values) {
            $groups[] = isset($values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]) ? $values[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY] : \Whisk\Recipes\Vendor\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
        }
    ...
    public function set_default_value($default_value)
    {
        foreach ($default_value as $index => $group) {
            if (!isset($group[static::TYPE_PROPERTY])) {
                $default_value[$index][static::TYPE_PROPERTY] = \Whisk\Recipes\Vendor\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
            }
        }
    ...
    public function to_json($load)
    {
        ...
        $value_data = array();
        foreach ($field_groups as $group_index => $fields) {
            $group = $this->get_group_by_name($fields[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
        ...
        $group_types = array();
        foreach ($this->groups as $group) {
            $group_types[] = array('name' => $group->get_name(), 'label' => $group->get_label());
        }
        $complex_data = \array_merge($complex_data, array('duplicate_groups_allowed' => $this->get_duplicate_groups_allowed(), 'group_types' => $group_types, 'layout' => $this->layout, 'labels' => $this->labels, 'min' => $this->get_min(), 'max' => $this->get_max(), 'multiple_groups' => \count($groups_data) > 1, 'groups' => $groups_data, 'value' => $value_data, 'collapsed' => $this->get_collapsed()));
        return $complex_data;
    }
    /**
    ...
    {
        $available_layouts = array(static::LAYOUT_GRID, static::LAYOUT_TABBED_HORIZONTAL, static::LAYOUT_TABBED_VERTICAL);
        if (!\in_array($layout, $available_layouts)) {
            ...
            \Whisk\Recipes\Vendor\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise($error_message);
            return $this;
        }
        $this->layout = $layout;
        return $this;
     ...
     * @return self  $this
     */
    public function set_collapsed($collapsed = \true)
    {
