...
use Whisk\Recipes\Vendor\WP_Query;
use Whisk\Recipes\Vendor\WP_Term_Query;
use Whisk\Recipes\Vendor\WP_User_Query;
use Whisk\Recipes\Vendor\WP_Comment_Query;
/**
 ...
 *  - Comments
 */
class Association_Field extends \Whisk\Recipes\Vendor\Carbon_Fields\Field\Field
{
    /**
    ...
    {
        $this->wp_toolset = \Whisk\Recipes\Vendor\Carbon_Fields\Carbon_Fields::resolve('wp_toolset');
        $this->set_value_set(new \Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::TYPE_VALUE_SET, array('type' => '', 'subtype' => '', 'id' => 0)));
    ...
    protected function value_string_to_property_array($value_string)
    {
        ...
        $property_array = array(\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $value_string, 'type' => $type, 'subtype' => $subtype, 'id' => \intval($id));
    ...
    protected function value_string_array_to_value_set($value_string_array)
    {
        ...
        foreach ($value_string_array as $raw_value_entry) {
            ...
            if (\is_array($raw_value_entry)) {
                ...
                $value_string = $raw_value_entry[\Whisk\Recipes\Vendor\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY];
            }
            $value_string = \trim($value_string);
    ...
    {
        global $wpdb;
        $args = wp_parse_args($args, array('page' => 1, 'term' => ''));
        $sql_queries = array();
        foreach ($this->types as $type) {
            $type_args = \array_merge($type, array('term' => $args['term']));
        ...
        return array('total_options' => $wpdb->get_var("SELECT COUNT(*) FROM (" . \preg_replace('~(LIMIT .*)$~', '', $sql_queries) . ") as t"), 'options' => $options);
    }
    /**
     * Get the types.
     *
     ...
     * @return self    $this
     */
    public function allow_duplicates($allow = \true)
    {
    ...
    protected function value_to_json()
    {
        ...
        foreach ($value_set as $entry) {
            $item = array('type' => $entry['type'], 'subtype' => $entry['subtype'], 'id' => \intval($entry['id']), 'title' => $this->get_title_by_type($entry['id'], $entry['type'], $entry['subtype']), 'label' => $this->get_item_label($entry['id'], $entry['type'], $entry['subtype']), 'is_trashed' => $entry['type'] == 'post' && get_post_status($entry['id']) === 'trash');
            $value[] = $item;
        }
        return $value;
    }
    ...
    public function to_json($load)
    {
        ...
        $field_data = \array_merge($field_data, array('value' => $this->value_to_json(), 'options' => $this->get_options(), 'min' => $this->get_min(), 'max' => $this->get_max(), 'duplicates_allowed' => $this->duplicates_allowed));
        return $field_data;
    }
    /**
    ...
    public function get_post_options_sql($args = array())
    {
        ...
        $args = apply_filters($filter_name, array('post_type' => $post_type, 'posts_per_page' => 1, 'fields' => 'ids', 'suppress_filters' => \false, 's' => $search_term));
        ...
        $posts_query = new \Whisk\Recipes\Vendor\WP_Query($args);
    ...
    public function get_term_options_sql($args = array())
    {
        ...
        $args = apply_filters($filter_name, array('hide_empty' => 0, 'taxonomy' => $taxonomy, 'fields' => 'count', 'number' => 1, 'search' => $search_term, 'update_term_meta_cache' => \false));
        ...
        $terms_query = new \Whisk\Recipes\Vendor\WP_Term_Query($args);
    ...
    public function get_user_options_sql($args = array())
    {
        ...
        $args = apply_filters($filter_name, array('fields' => 'ID', 'number' => 1, 'search' => $search_term));
        $users_query = new \Whisk\Recipes\Vendor\WP_User_Query();
    ...
    public function get_comment_options_sql($args = array())
    {
         ...
         */
        $filter_name = 'carbon_fields_association_field_options_' . $this->get_base_name() . '_' . $type;
        $args = apply_filters($filter_name, array('fields' => 'ids', 'number' => 1, 'search' => $search_term));
        add_filter('comments_clauses', array($this, 'get_comments_clauses'));
        $comments_query = new \Whisk\Recipes\Vendor\WP_Comment_Query();
    ...
    {
        return array('id' => \intval($data->ID), 'title' => $this->get_title_by_type($data->ID, $data->type, $data->subtype), 'thumbnail' => get_the_post_thumbnail_url($data->ID, 'thumbnail'), 'type' => $data->type, 'subtype' => $data->subtype, 'label' => $this->get_item_label($data->ID, $data->type, $data->subtype), 'is_trashed' => get_post_status($data->ID) == 'trash', 'edit_link' => $this->get_object_edit_link(\get_object_vars($data), $data->ID));
    }
    /**
    ...
    {
        return array('id' => \intval($data->ID), 'title' => $data->title, 'thumbnail' => '', 'type' => $data->type, 'subtype' => $data->subtype, 'label' => $this->get_item_label($data->ID, $data->type, $data->subtype), 'is_trashed' => \false, 'edit_link' => $this->get_object_edit_link(\get_object_vars($data), $data->ID));
    }
    /**
    ...
    {
        return array('id' => \intval($data->ID), 'title' => $this->get_title_by_type($data->ID, 'comment'), 'thumbnail' => '', 'type' => 'comment', 'subtype' => 'comment', 'label' => $this->get_item_label($data->ID, 'comment'), 'is_trashed' => \false, 'edit_link' => $this->get_object_edit_link(\get_object_vars($data), $data->ID));
    }
    /**
    ...
    {
        return array('id' => \intval($data->ID), 'title' => $this->get_title_by_type($data->ID, 'user'), 'thumbnail' => get_avatar_url($data->ID, array('size' => 150)), 'type' => 'user', 'subtype' => 'user', 'label' => $this->get_item_label($data->ID, 'user'), 'is_trashed' => \false, 'edit_link' => $this->get_object_edit_link(\get_object_vars($data), $data->ID));
    }
}
