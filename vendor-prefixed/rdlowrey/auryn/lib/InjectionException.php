...
class InjectionException extends \Whisk\Recipes\Vendor\Auryn\InjectorException
{
    ...
    public static function fromInvalidCallable(array $inProgressMakes, $callableOrMethodStr, \Exception $previous = null)
    {
        ...
        if (\is_string($callableOrMethodStr)) {
            ...
            if (\is_array($callableOrMethodStr) && \array_key_exists(0, $callableOrMethodStr) && \array_key_exists(0, $callableOrMethodStr)) {
                if (\is_string($callableOrMethodStr[0]) && \is_string($callableOrMethodStr[1])) {
                    ...
                    if (\is_object($callableOrMethodStr[0]) && \is_string($callableOrMethodStr[1])) {
                        $callableString .= \sprintf("[object(%s), '%s']", \get_class($callableOrMethodStr[0]), $callableOrMethodStr[1]);
                    }
                }
            }
        }
        if ($callableString) {
            ...
            $message = \sprintf("%s. Invalid callable was '%s'", \Whisk\Recipes\Vendor\Auryn\Injector::M_INVOKABLE, $callableString);
        } else {
            $message = \Whisk\Recipes\Vendor\Auryn\Injector::M_INVOKABLE;
        }
        return new self($inProgressMakes, $message, \Whisk\Recipes\Vendor\Auryn\Injector::E_INVOKABLE, $previous);
    }
    /**
