...
class Injector
{
    ...
    public function __construct(\Whisk\Recipes\Vendor\Auryn\Reflector $reflector = null)
    {
        $this->reflector = $reflector ?: new \Whisk\Recipes\Vendor\Auryn\CachingReflector();
    }
    ...
    public function alias($original, $alias)
    {
        if (empty($original) || !\is_string($original)) {
            throw new \Whisk\Recipes\Vendor\Auryn\ConfigException(self::M_NON_EMPTY_STRING_ALIAS, self::E_NON_EMPTY_STRING_ALIAS);
        }
        ...
        if (isset($this->shares[$originalNormalized])) {
            throw new \Whisk\Recipes\Vendor\Auryn\ConfigException(\sprintf(self::M_SHARED_CANNOT_ALIAS, $this->normalizeName(\get_class($this->shares[$originalNormalized])), $alias), self::E_SHARED_CANNOT_ALIAS);
        }
    ...
    public function share($nameOrInstance)
    {
        if (\is_string($nameOrInstance)) {
            ...
            throw new \Whisk\Recipes\Vendor\Auryn\ConfigException(\sprintf(self::M_SHARE_ARGUMENT, __CLASS__, \gettype($nameOrInstance)), self::E_SHARE_ARGUMENT);
        }
        return $this;
    }
    private function shareClass($nameOrInstance)
    {
        ...
        $this->shares[$normalizedName] = isset($this->shares[$normalizedName]) ? $this->shares[$normalizedName] : null;
    }
    ...
    private function shareInstance($obj)
    {
        ...
        if (isset($this->aliases[$normalizedName])) {
            ...
            throw new \Whisk\Recipes\Vendor\Auryn\ConfigException(\sprintf(self::M_ALIASED_CANNOT_SHARE, $normalizedName, $this->aliases[$normalizedName]), self::E_ALIASED_CANNOT_SHARE);
        }
    ...
    public function prepare($name, $callableOrMethodStr)
    {
        if ($this->isExecutable($callableOrMethodStr) === \false) {
            throw \Whisk\Recipes\Vendor\Auryn\InjectionException::fromInvalidCallable($this->inProgressMakes, $callableOrMethodStr);
        }
        list(, $normalizedName) = $this->resolveAlias($name);
    ...
    public function delegate($name, $callableOrMethodStr)
    {
        if ($this->isExecutable($callableOrMethodStr) === \false) {
            ...
            } elseif (\is_array($callableOrMethodStr) && \count($callableOrMethodStr) === 2 && \array_key_exists(0, $callableOrMethodStr) && \array_key_exists(1, $callableOrMethodStr)) {
            ...
            throw new \Whisk\Recipes\Vendor\Auryn\ConfigException(\sprintf(self::M_DELEGATE_ARGUMENT, __CLASS__, $errorDetail), self::E_DELEGATE_ARGUMENT);
        }
        $normalizedName = $this->normalizeName($name);
    ...
    public function inspect($nameFilter = null, $typeFilter = null)
    {
        ...
        $types = array(self::I_BINDINGS => "classDefinitions", self::I_DELEGATES => "delegates", self::I_PREPARES => "prepares", self::I_ALIASES => "aliases", self::I_SHARES => "shares");
    ...
    public function make($name, array $args = array())
    {
        ...
        if (isset($this->inProgressMakes[$normalizedClass])) {
            throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes, \sprintf(self::M_CYCLIC_DEPENDENCY, $className), self::E_CYCLIC_DEPENDENCY);
        }
    ...
    private function provisionInstance($className, $normalizedClass, array $definition)
    {
        try {
            ...
            if (!$ctor) {
                ...
                throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes, \sprintf(self::M_NON_PUBLIC_CONSTRUCTOR, $className), self::E_NON_PUBLIC_CONSTRUCTOR);
            } elseif ($ctorParams = $this->reflector->getCtorParams($className)) {
                $reflClass = $this->reflector->getClass($className);
                $definition = isset($this->classDefinitions[$normalizedClass]) ? \array_replace($this->classDefinitions[$normalizedClass], $definition) : $definition;
            ...
            throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes, \sprintf(self::M_MAKE_FAILURE, $className, $e->getMessage()), self::E_MAKE_FAILURE, $e);
        }
    }
    private function instantiateWithoutCtorParams($className)
    {
        ...
        if (!$reflClass->isInstantiable()) {
            ...
            throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes, \sprintf(self::M_NEEDS_DEFINITION, $type, $className), self::E_NEEDS_DEFINITION);
        }
    ...
    private function buildArgFromParamDefineArr($definition)
    {
        if (!\is_array($definition)) {
            throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes);
        }
    ...
    private function buildArgFromReflParam(\ReflectionParameter $reflParam, $className = null)
    {
        if (\array_key_exists($reflParam->name, $this->paramDefinitions)) {
            ...
            $classDeclare = $reflFunc instanceof \ReflectionMethod ? " declared in " . $reflFunc->getDeclaringClass()->name . "::" : "";
            $classWord = $reflFunc instanceof \ReflectionMethod ? $className . '::' : '';
            $funcWord = $classWord . $reflFunc->name;
            throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes, \sprintf(self::M_UNDEFINED_PARAM, $reflParam->name, $reflParam->getPosition(), $funcWord, $classDeclare), self::E_UNDEFINED_PARAM);
        }
        return $arg;
    }
    private function prepareInstance($obj, $normalizedClass)
    {
        ...
        if ($interfaces === \false) {
            throw new \Whisk\Recipes\Vendor\Auryn\InjectionException($this->inProgressMakes, \sprintf(self::M_MAKING_FAILED, $normalizedClass, \gettype($obj)), self::E_MAKING_FAILED);
        }
        if (empty($interfaces)) {
            return $obj;
        }
    ...
    public function execute($callableOrMethodStr, array $args = array())
    {
        ...
        $executable = new \Whisk\Recipes\Vendor\Auryn\Executable($reflFunc, $invocationObj);
    ...
    public function buildExecutable($callableOrMethodStr)
    {
        try {
            ...
            throw \Whisk\Recipes\Vendor\Auryn\InjectionException::fromInvalidCallable($this->inProgressMakes, $callableOrMethodStr, $e);
        }
        return new \Whisk\Recipes\Vendor\Auryn\Executable($reflFunc, $invocationObj);
    }
    private function buildExecutableStruct($callableOrMethodStr)
    {
        ...
        } elseif (\is_array($callableOrMethodStr) && isset($callableOrMethodStr[0], $callableOrMethodStr[1]) && \count($callableOrMethodStr) === 2) {
    ...
    private function buildExecutableStructFromString($stringExecutable)
    {
        if (\function_exists($stringExecutable)) {
            ...
            throw \Whisk\Recipes\Vendor\Auryn\InjectionException::fromInvalidCallable($this->inProgressMakes, $stringExecutable);
        }
        return $executableStruct;
    }
    ...
    private function buildExecutableStructFromArray($arrayExecutable)
    {
        ...
        if (\is_object($classOrObj) && \method_exists($classOrObj, $method)) {
            ...
            throw \Whisk\Recipes\Vendor\Auryn\InjectionException::fromInvalidCallable($this->inProgressMakes, $arrayExecutable);
        }
        return $executableStruct;
    }
}
